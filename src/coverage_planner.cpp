...
private:
  ...
  typedef std::vector<std::vector<iroc_mission_handler::Waypoint>>
      coverage_paths_t;
  ...
  algorithm_config_t
  parse_algorithm_config(mrs_lib::ParamLoader &param_loader) const;

  coverage_paths_t
  getCoveragePaths(const iroc_fleet_manager::CoverageMission &mission) const;
};

bool CoveragePlanner::initialize(
  ...
  param_loader.addYamlFile(ros::package::getPath("iroc_fleet_manager") +
...
CoveragePlanner::createGoal(const std::string &goal) const {
  ...
  latlon_origin_msg.y =
      common_handlers_->handlers->robots_map[robots_msg.at(0).name]
          .safety_area_info->safety_area.origin_y;
...
algorithm_config_t CoveragePlanner::parse_algorithm_config(
  ...
  param_loader.loadParam(yaml_prefix + "drone_mass",
                         algorithm_config.energy_calculator_config.drone_mass);
  param_loader.loadParam(yaml_prefix + "drone_area",
                         algorithm_config.energy_calculator_config.drone_area);
  param_loader.loadParam(
      yaml_prefix + "average_acceleration",
      algorithm_config.energy_calculator_config.average_acceleration);
  param_loader.loadParam(
      yaml_prefix + "propeller_radius",
      algorithm_config.energy_calculator_config.propeller_radius);
  param_loader.loadParam(
      yaml_prefix + "number_of_propellers",
      algorithm_config.energy_calculator_config.number_of_propellers);
  param_loader.loadParam(
      yaml_prefix + "allowed_path_deviation",
      algorithm_config.energy_calculator_config.allowed_path_deviation);
  param_loader.loadParam(yaml_prefix + "number_of_rotations",
                         algorithm_config.number_of_rotations);

  // Load battery model parameters
  const std::string battery_prefix = yaml_prefix + "battery_model/";
  param_loader.loadParam(
      battery_prefix + "cell_capacity",
      algorithm_config.energy_calculator_config.battery_model.cell_capacity);
  param_loader.loadParam(
      battery_prefix + "number_of_cells",
      algorithm_config.energy_calculator_config.battery_model.number_of_cells);
  param_loader.loadParam(
      battery_prefix + "d0",
      algorithm_config.energy_calculator_config.battery_model.d0);
  param_loader.loadParam(
      battery_prefix + "d1",
      algorithm_config.energy_calculator_config.battery_model.d1);
  param_loader.loadParam(
      battery_prefix + "d2",
      algorithm_config.energy_calculator_config.battery_model.d2);
  param_loader.loadParam(
      battery_prefix + "d3",
      algorithm_config.energy_calculator_config.battery_model.d3);

  // Load speed model parameters
  const std::string speed_prefix = yaml_prefix + "best_speed_model/";
  param_loader.loadParam(
      speed_prefix + "c0",
      algorithm_config.energy_calculator_config.best_speed_model.c0);
  param_loader.loadParam(
      speed_prefix + "c1",
      algorithm_config.energy_calculator_config.best_speed_model.c1);
  param_loader.loadParam(
      speed_prefix + "c2",
      algorithm_config.energy_calculator_config.best_speed_model.c2);

  // Load coordinate system parameters
  param_loader.loadParam(yaml_prefix + "points_in_lat_lon",
                         algorithm_config.points_in_lat_lon);
  if (algorithm_config.points_in_lat_lon) {
    param_loader.loadParam(yaml_prefix + "latitude_origin",
                           algorithm_config.lat_lon_origin.first);
    param_loader.loadParam(yaml_prefix + "longitude_origin",
                           algorithm_config.lat_lon_origin.second);
  }

  param_loader.loadParam(yaml_prefix + "sweeping_step",
                         algorithm_config.sweeping_step);

  int decomposition_method;
  param_loader.loadParam(yaml_prefix + "decomposition_method",
                         decomposition_method);
  algorithm_config.decomposition_type =
      static_cast<decomposition_type_t>(decomposition_method);

  param_loader.loadParam(yaml_prefix + "min_sub_polygons_per_uav",
                         algorithm_config.min_sub_polygons_per_uav);

  // Load optimization parameters
  param_loader.loadParam(yaml_prefix + "rotations_per_cell",
                         algorithm_config.rotations_per_cell);
  param_loader.loadParam(yaml_prefix + "no_improvement_cycles_before_stop",
                         algorithm_config.no_improvement_cycles_before_stop);
  param_loader.loadParam(yaml_prefix + "max_single_path_energy",
                         algorithm_config.max_single_path_energy);
...
std::vector<MapPolygon>
assign_closest_polygons(const std::vector<point_t> &uav_positions,
...
CoveragePlanner::coverage_paths_t CoveragePlanner::getCoveragePaths(
  ...
  MapPolygon polygon =
      MapPolygon(fly_zone, no_fly_zones, planner_config_.lat_lon_origin);
  ...
  auto decomposed_polygon =
      decompose_polygon(mission.robots.size(), planner_config_, polygon);

  std::vector<point_t> polygon_centroids;
  for (const auto &polygon : decomposed_polygon) {
    auto points_tmp = polygon.get_all_points();
    auto m_polygon_points =
        std::vector<point_t>{points_tmp.begin(), points_tmp.end()};
  ...
  for (const auto &polygon : decomposed_polygon) {
    ...
    try {
      ...
      planner_config_.start_pos.first =
          mission.robots.at(uav_index).global_position.x;
      planner_config_.start_pos.second =
          mission.robots.at(uav_index).global_position.y;
      auto f = [&](int n) {
        return solve_for_uavs(n, planner_config_, assigned_polygon,
                              energy_calculator, shortest_path_calculator,
                              shared_logger);
      };
      best_solution = generate_with_constraints(
          planner_config_.max_single_path_energy * 3600, 1, f);
      uav_index++;
    ...
    for (auto &path : best_paths) {
      ...
      for (auto &p : path) {
        ...
        auto lat_lon_p = meters_to_gps_coordinates(
            {p.x, p.y}, planner_config_.lat_lon_origin);
        p.x = lat_lon_p.first;
...
PLUGINLIB_EXPORT_CLASS(iroc_fleet_manager::coverage_planner::CoveragePlanner,
