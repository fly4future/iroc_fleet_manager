...
void IROCFleetManager::initialize() {
  ...
  // planner_loader_ = std::make_unique<pluginlib::ClassLoader<iroc_fleet_manager::planners::Planner>>("iroc_fleet_manager",
...
void IROCFleetManager::timerMain() {
  ...
  bool any_failure     = false;
  {
    ...
    if (any_failure) {
      RCLCPP_WARN(node_->get_logger(), " Early failure detected, not all robots finished successfully, aborting mission.");
      // TODO
...
bool IROCFleetManager::getWorldOriginCallback(const std::shared_ptr<iroc_fleet_manager::srv::GetWorldOriginSrv::Request> &request,
  ...
  for (const auto &rh : robot_handlers_.handlers) {
    ...
    if (!rh.sh_safety_area_info.hasMsg()) {
      response->message = "No safety area info received, check if the Safety Area Manager is running!";
      response->success = false;
      return true;
    }
...
//   //
//   // if (result->success) {
//   //   ROS_INFO_STREAM("[IROCFleetManager]: Action server on robot " << robot_name << " finished with state: \"" << state.toString() << "\". Result message
//   is: \""
//   //                                                                 << result->message << "\"");
//   // } else {
//   //   ROS_WARN_STREAM("[IROCFleetManager]: Action server on robot " << robot_name << " finished with state: \"" << state.toString() << "\". Result message
//   is: \""
//   //                                                                 << result->message << "\"");
//   // }
//   //
...
//   if (sc.call(req, res)) {
//     if (response->success) {
//       ROS_INFO_STREAM("Called service \"" << sc.getService() << "\" with response \"" << response->message << "\".");
//       return {true, res.message};
//     } else {
//       ROS_WARN_STREAM("Called service \"" << sc.getService() << "\" with response \"" << response->message << "\".");
