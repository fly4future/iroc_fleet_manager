#include <iroc_fleet_manager/iroc_plugins/cz_altitude_coverage_planner.h>
...
bool CzAltitudeCoveragePlanner::initialize(const ros::NodeHandle &parent_nh, const std::string &name, const std::string &name_space,
                                 std::shared_ptr<iroc_fleet_manager::CommonHandlers_t> common_handlers) {
  // nh_ will behave just like normal NodeHandle
  ros::NodeHandle nh_(parent_nh, name_space);
  ros::NodeHandle nh2_(parent_nh, name_space);
  ...
  /* load parameters */
  mrs_lib::ParamLoader param_loader(nh_, "CzAltitudeCoveragePlanner");
...
bool CzAltitudeCoveragePlanner::activate(void) {
...
void CzAltitudeCoveragePlanner::deactivate(void) {
...
std::tuple<result_t, std::vector<iroc_mission_handler::MissionGoal>> CzAltitudeCoveragePlanner::createGoal(const std::string &goal) const {
  // Goal to be filled
  std::vector<iroc_mission_handler::MissionGoal> mission_robots;
  ROS_INFO("[CzAltitudeCoveragePlanner] Received goal :%s ",
  ...
  for (const auto &robot : robots) {
    ...
    if (!isRobotInFleet) {
      ROS_WARN("[CzAltitudeCoveragePlanner] Robot %s not within the fleet", name.c_str());
      std::stringstream ss;
  ...
  try {
    ...
    for (std::vector<iroc_mission_handler::Waypoint> drone_path : paths) {
      ...
      double lat_last = 0, long_last = 0;
      int i = 0;
      for (iroc_mission_handler::Waypoint waypoint : drone_path) {
        if (i > 0) {
          double dist = distanceLatLon(waypoint.reference.position.x, waypoint.reference.position.y, lat_last, long_last);
          int num_new_points = std::floor(dist / 15.0);

          // If dist is exactly multiple of 15, then the last duplicite point is not added 
          if (std::abs(dist - num_new_points * 15.0) < 1e-6) {
            num_new_points -= 1;
          }

          if (num_new_points > 0) {
            for (int j = 1; j <= num_new_points; j++) {
              iroc_mission_handler::Waypoint new_wp;
              double ratio = (15.0 * j) / dist;
              new_wp.reference.position.x = lat_last + (waypoint.reference.position.x - lat_last) * ratio;
              new_wp.reference.position.y = long_last + (waypoint.reference.position.y - long_last) * ratio;
              std::pair<double, std::string> res;
              try {
                res = elev_grid.getEllipsoidElevation(new_wp.reference.position.x, new_wp.reference.position.y);
              } catch (const std::exception& e) {
                ROS_ERROR("[%s]: Error for point: %s", name_.c_str(), e.what());
                return endWithError(result, e.what(), mission_robots);
              }
              new_wp.reference.position.z = res.first + height;    // Altitude above sea level plus the drone's height above ground.
              new_wp.reference.heading = waypoint.reference.heading;
              new_drone_path.push_back(new_wp);
            }
          }
        }

        lat_last = waypoint.reference.position.x;
        long_last = waypoint.reference.position.y;
        std::pair<double, std::string> res;
        try {
          res = elev_grid.getEllipsoidElevation(waypoint.reference.position.x, waypoint.reference.position.y);
        } catch (const std::exception& e) {
          ROS_ERROR("[%s]: Error for point: %s", name_.c_str(), e.what());
          return endWithError(result, e.what(), mission_robots);
        }
        waypoint.reference.position.z = res.first + height;    // Altitude above sea level plus the drone's height above ground.
        new_drone_path.push_back(waypoint);
        i += 1;
      }
      new_paths.push_back(new_drone_path);
    }

  } catch (const std::exception& e) {
    ROS_ERROR("[%s]: Critical error: %s", name_.c_str(), e.what());
    return endWithError(result, e.what(), mission_robots);
  }
  ...
  for (int it = 0; it < mission.robots.size(); it++) {
    ...
    robot.points          = new_paths[it];
    robot.terminal_action = mission.robots[it].terminal_action;
    robot.height_id       = iroc_mission_handler::MissionGoal::FRAME_ID_LATLON;
    robot.frame_id        = mission.robots[it].frame_id;
    mission_robots.push_back(robot);
  }

  ROS_INFO("[CzAltitudeCoveragePlanner] Goal created successfully!");
  result.success = true;
...
algorithm_config_t CzAltitudeCoveragePlanner::parse_algorithm_config(mrs_lib::ParamLoader &param_loader) const {
...
std::tuple<result_t, std::vector<iroc_mission_handler::MissionGoal>> CzAltitudeCoveragePlanner::endWithError(result_t result, const std::string& error_msg, std::vector<iroc_mission_handler::MissionGoal> mission_robots) const {
  result.message = error_msg;
  result.success = false;
  return std::make_tuple(result, mission_robots);
}

double CzAltitudeCoveragePlanner::toRadians(double degree) const {
    return degree * (M_PI / 180.0);
}

double CzAltitudeCoveragePlanner::distanceLatLon(double lat1, double lon1, double lat2, double lon2) const {
    // Poloměr Země v kilometrech (střední hodnota)
    const double R = 6371.0;

    // Převod souřadnic na radiány
...
CzAltitudeCoveragePlanner::coverage_paths_t CzAltitudeCoveragePlanner::getCoveragePaths(const iroc_fleet_manager::CoverageMission &mission) const {
  ...
  ROS_INFO_STREAM("[CzAltitudeCoveragePlanner:]: Energy calculator created. Optimal speed: " << energy_calculator.get_optimal_speed());
  ...
  ROS_INFO("[CzAltitudeCoveragePlanner:]: Size of decomposed polygon: %zu", decomposed_polygon.size());
  ...
  for (const auto &polygon : decomposed_polygon) {
    ...
    try {
      ...
      ROS_WARN_STREAM("[CzAltitudeCoveragePlanner:]: Error while decomposing the polygon");
...
PLUGINLIB_EXPORT_CLASS(iroc_fleet_manager::planners::cz_altitude_coverage_planner::CzAltitudeCoveragePlanner, iroc_fleet_manager::planners::Planner);
