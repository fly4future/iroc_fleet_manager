...
MapPolygon MapPolygon::rotated(double angle) const
{
  ...
  std::transform(fly_zone_polygon_points.begin(), fly_zone_polygon_points.end(),
  ...
  std::transform(no_fly_zone_polygons.begin(), no_fly_zone_polygons.end(),
                 std::inserter(new_polygon.no_fly_zone_polygons, new_polygon.no_fly_zone_polygons.begin()), [angle](const auto& polygon) {
                   polygon_t no_fly_new_polygon;
                   std::transform(polygon.begin(), polygon.end(), std::inserter(no_fly_new_polygon, no_fly_new_polygon.begin()),
                                  [angle](const auto& p) { return rotate_point(p, angle); });
...
std::vector<double> MapPolygon::get_n_longest_edges_rotation_angles(size_t n) const
{
  ...
  std::sort(edges.begin(), edges.end(), [](const segment_t& s1, const segment_t& s2) { return segment_length(s1) > segment_length(s2); });
...
double MapPolygon::area() const
{
  ...
  {
    whole_area +=
        (fly_zone_polygon_points[i - 1].first + fly_zone_polygon_points[i].first) * (fly_zone_polygon_points[i - 1].second - fly_zone_polygon_points[i].second);
  }
...
std::pair<MapPolygon, MapPolygon> MapPolygon::split_by_vertical_line(double x)
{
  ...
  for (size_t i = 0; i + 1 < fly_zone_copy.size(); ++i)
  {
    ...
    if (fly_zone_copy[i + 1].first != x
        && ((fly_zone_copy[i].first < x && fly_zone_copy[i + 1].first > x) || (fly_zone_copy[i].first > x && fly_zone_copy[i + 1].first < x)))
    {
