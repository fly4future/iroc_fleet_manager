...
EnergyCalculator::EnergyCalculator(const energy_calculator_config_t& energy_calculator_config, std::shared_ptr<loggers::SimpleLogger> logger)
    : m_logger(std::move(logger)), config(energy_calculator_config)
{
  ...
  double C_eff_r = config.battery_model.cell_capacity
                   * (config.battery_model.d0 + config.battery_model.d1 * P_cell_r + config.battery_model.d2 * std::pow(P_cell_r, 2)
...
/* calculate_straight_line_energy() //{ */
...
double EnergyCalculator::calculate_straight_line_energy(double v_in, double a_in, double v_out, double a_out, double s_tot) const
{
  ...
  if (s_acc + s_dec <= s_tot)
  {
    ...
    return ((s_tot - s_acc - s_dec) / v_r) * P_r + calculate_acceleration_energy(v_in, v_r, t_acc) + calculate_acceleration_energy(v_out, v_r, t_dec);
  } else
  {
...
/* calculate_straight_line_energy_between_turns() //{ */

double EnergyCalculator::calculate_straight_line_energy_between_turns(const turning_properties_t& turn1, const turning_properties_t& turn2, double s_tot) const
{
  ...
  if (s_acc_slow + s_dec_slow < s_tot)
  {
    ...
    return calculate_straight_line_energy(turn1.v_after, config.average_acceleration, turn2.v_before, -config.average_acceleration,
                                          s_tot - s_acc_slow - s_dec_slow)
           + slow_acceleration_energy;
  } else
  {
    // If the UAV can only start the slow deceleration after the slow acceleration
    return calculate_short_line_energy(turn1.d_vym, turn1.a_after, turn2.d_vym, turn2.a_before, s_tot);
  }
}
//}

/* calculate_short_line_energy() //{ */
...
double EnergyCalculator::calculate_short_line_energy(double v_in, double a_in, double v_out, double a_out, double s) const
{
  ...
  auto v_sol = std::sqrt((v_in * v_in / a_in + v_out * v_out / std::abs(a_out) + 2 * s) / (1 / a_in + 1 / std::abs(a_out)));
