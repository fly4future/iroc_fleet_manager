...
/*!
 * Template abstract class for fleet manager missions.
 *
...
public:
  // Type aliases: GoalType, FeedbackType and ResultType are taken from the
  // ActionType
  ...
  using FeedbackType =
      typename ActionType::_action_feedback_type::_feedback_type;
...
protected:
  /*!
   * Abstract class for fleet manager missions; To be overriden by
   * child classes
   *
   * @tparam ActionType Type representing the action/mission type for the fleet
   */
  ...
  bool changeRobotMissionStateCallback(
      iroc_fleet_manager::ChangeRobotMissionStateSrv::Request &req,
      iroc_fleet_manager::ChangeRobotMissionStateSrv::Response &res);

  // action client
  void missionActiveCallback(const std::string &robot_name) const;
  void
  missionDoneCallback(const actionlib::SimpleClientGoalState &state,
                      const iroc_mission_handler::MissionResultConstPtr &result,
                      const std::string &robot_name);
  void missionFeedbackCallback(
      const iroc_mission_handler::MissionFeedbackConstPtr &feedback,
      const std::string &robot_name);
...
template <typename ActionType> void BaseFleetManager<ActionType>::onInit() {
  ...
  timer_main_ = nh_.createTimer(ros::Rate(main_timer_rate),
                                &BaseFleetManager::timerMain, this);
  timer_feedback_ = nh_.createTimer(ros::Rate(feedback_timer_rate),
                                    &BaseFleetManager::timerFeedback, this);

  // | --------------------- service servers -------------------- |

  ss_change_fleet_mission_state_ = nh_.advertiseService(
      nh_.resolveName("svc/change_fleet_mission_state"),
      &BaseFleetManager::changeFleetMissionStateCallback, this);
  ROS_INFO("[IROCAutonomyTestManager]: Created ServiceServer on service "
           "\'svc_server/change_mission_state\' -> \'%s\'",
           ss_change_fleet_mission_state_.getService().c_str());
  ss_change_robot_mission_state_ = nh_.advertiseService(
      nh_.resolveName("svc/change_robot_mission_state"),
  ...
  action_server_ptr_ =
      std::make_unique<ActionServer_T>(nh_, ros::this_node::getName(), false);
  action_server_ptr_->registerGoalCallback(
      boost::bind(&BaseFleetManager::actionCallbackGoal, this));
  action_server_ptr_->registerPreemptCallback(
      boost::bind(&BaseFleetManager::actionCallbackPreempt, this));
 ...
 */
template <typename ActionType>
void BaseFleetManager<ActionType>::timerMain(
 ...
 */
template <typename ActionType>
void BaseFleetManager<ActionType>::timerFeedback(
    [[maybe_unused]] const ros::TimerEvent &event) {
 ...
 */
template <typename ActionType>
bool BaseFleetManager<ActionType>::changeFleetMissionStateCallback(
    mrs_msgs::String::Request &req, mrs_msgs::String::Response &res) {
  active_mission_change_ = true;
  std::scoped_lock lock(action_server_mutex_, fleet_mission_handlers_.mtx);

  ROS_INFO_STREAM("[IROCFleetManager]: Received a "
                  << req.value << " request for the fleet");
  std::stringstream ss;
  bool success = true;
  if (action_server_ptr_->isActive()) {
    if (req.value == "start") {
      ...
      for (auto &rh : fleet_mission_handlers_.handlers) {
        const auto resp =
            callService<std_srvs::Trigger>(rh.sc_robot_activation);
        if (!resp.success) {
          success = false;
          ROS_WARN_STREAM("[IROCFleetManager]: "
                          << "Call for robot \"" << rh.robot_name
                          << "\" was not successful with message: "
                          << resp.message << "\n");
          ss << "Call for robot \"" << rh.robot_name
      ...
      for (auto &rh : fleet_mission_handlers_.handlers) {
        ...
        if (action_client_state.isDone()) {
          ...
          ROS_WARN_STREAM("[IROCFleetManager]: Robot \""
                          << rh.robot_name << "\" mission done. Skipping.");
        } else {
          ROS_INFO_STREAM("[IROCFleetManager]: Cancelling \"" << rh.robot_name
                                                              << "\" mission.");
 ...
 */
template <typename ActionType>
bool BaseFleetManager<ActionType>::changeRobotMissionStateCallback(
    iroc_fleet_manager::ChangeRobotMissionStateSrv::Request &req,
    iroc_fleet_manager::ChangeRobotMissionStateSrv::Response &res) {
  ...
  if (action_server_ptr_->isActive()) {
    if (req.type == "start") {
      ROS_INFO_STREAM("[IROCFleetManager]: Calling mission pausing for robot: "
                      << req.robot_name << ".");
      const auto resp =
          callService<std_srvs::Trigger>(rh_ptr->sc_robot_activation);
      if (!resp.success) {
        success = false;
        ROS_WARN_STREAM("[IROCFleetManager]: "
                        << "Call for robot \"" << req.robot_name
                        << "\" was not successful with message: "
                        << resp.message << "\n");
        ss << "Call for robot \"" << req.robot_name
           << "\" was not successful with message: " << resp.message << "\n";
      } else {
        ss << "Call successful.\n";
      }
      ...
      ROS_INFO_STREAM("[IROCFleetManager]: Calling mission pausing for robot: "
                      << req.robot_name << ".");
      const auto resp =
          callService<std_srvs::Trigger>(rh_ptr->sc_robot_pausing);
      if (!resp.success) {
        success = false;
        ROS_WARN_STREAM("[IROCFleetManager]: "
                        << "Call for robot \"" << req.robot_name
      ...
      if (action_client_state.isDone()) {
        ...
        success = false;
        ROS_WARN_STREAM("[IROCFleetManager]: Robot \""
                        << rh_ptr->robot_name << "\" mission done. Skipping.");
      } else {
        ss << "Call successful.\n";
...
template <typename ActionType>
void BaseFleetManager<ActionType>::missionActiveCallback(
    const std::string &robot_name) const {
  ROS_INFO_STREAM("[IROCFleetManager]: Action server on robot "
                  << robot_name << " is processing the goal.");
}
 ...
 */
template <typename ActionType>
void BaseFleetManager<ActionType>::missionDoneCallback(
 ...
 */
template <typename ActionType>
void BaseFleetManager<ActionType>::missionFeedbackCallback(
    const iroc_mission_handler::MissionFeedbackConstPtr &feedback,
    const std::string &robot_name) {
...
void BaseFleetManager<ActionType>::actionCallbackGoal() {
  std::scoped_lock lock(action_server_mutex_);
  boost::shared_ptr<const GoalType> new_action_server_goal =
      action_server_ptr_->acceptNewGoal();
  ...
  if (!all_success) {
    ...
    for (const auto &result : results) {
      ...
      if (!result.second.success) {
        ...
        ROS_WARN_STREAM(
            "[IROCFleetManager]: Failure starting robot clients: " << ss.str());
      }
    }
    action_server_result.success = false;
...
template <typename ActionType>
template <typename MissionRobots_T>
std::map<std::string, typename BaseFleetManager<ActionType>::result_t>
BaseFleetManager<ActionType>::sendRobotGoals(const MissionRobots_T &robots) {
  std::scoped_lock lck(fleet_mission_handlers_.mtx);
...

  {
    ...
    for (const auto &robot : robots) {
      bool success = true;
      std::stringstream ss;
      const std::string action_client_topic =
          "/" + robot.name + nh_.resolveName("ac/waypoint_mission");
      auto action_client_ptr =
      ...
      if (!action_client_ptr->waitForServer(ros::Duration(5.0))) {
        ...
        ss << "Action server from robot: " + robot.name +
                  " failed to connect. Check the iroc_mission_handler node.\n";
      ...
      if (!action_client_ptr->isServerConnected()) {
        ...
        ROS_WARN_STREAM(
            "[IROCFleetManager]: Action server from robot :" + robot.name +
            " is not connected. Check the iroc_mission_handler node.");
        robot_results[robot.name].message = ss.str();
        robot_results[robot.name].success = false;
        success = false;
      ...
      action_client_ptr->sendGoal(
          action_goal,
          [this, robot_name = robot.name](const auto &state,
                                          const auto &result) {
      ...
      const std::string mission_activation_client_topic =
          "/" + robot.name + nh_.resolveName("svc/mission_activation");
      ros::ServiceClient sc_robot_activation =
      ...
      const std::string mission_pausing_client_topic =
          "/" + robot.name + nh_.resolveName("svc/mission_pausing");
      ros::ServiceClient sc_robot_pausing =
...
BaseFleetManager<ActionType>::processAggregatedFeedbackInfo(
  ...
  auto mission_progress =
      std::accumulate(robots_progress.begin(), robots_progress.end(), 0.0) /
      robots_progress.size();
...
BaseFleetManager<ActionType>::processFeedbackMsg() const {
  ...
  if (any_idle)
    return std::make_tuple(
        "One robot is idle, check mission_manager",
 ...
 */
template <typename ActionType>
typename BaseFleetManager<ActionType>::robot_mission_handler_t *
BaseFleetManager<ActionType>::findRobotHandler(
    const std::string &robot_name,
...
void BaseFleetManager<ActionType>::cancelRobotClients() {

  {
    ...
    for (auto &rh : fleet_mission_handlers_.handlers) {
      ...
      bool lost_robot =
          std::any_of(lost_robot_names_.begin(), lost_robot_names_.end(),
                      [&rh](const auto &lost_robot) {
                        return rh.robot_name == lost_robot;
                      });

      bool robot_got_result = std::any_of(
          fleet_mission_handlers_.handlers.begin(),
          fleet_mission_handlers_.handlers.end(), [&rh](const auto &handler) {
            return rh.robot_name == handler.robot_name && handler.got_result;
          });

      if (lost_robot || robot_got_result) {
        ROS_INFO_STREAM("[IROCFleetManager]: Robot \""
                        << rh.robot_name
                        << "\" with no active mission, skipping cancel...");
        continue;
      }
      ...
      if (!action_client_state.isDone()) {
        ROS_INFO_STREAM("[IROCFleetManager]: Robot \""
                        << rh.robot_name
                        << "\" has an active mission, cancelling...");
        rh.action_client_ptr->cancelGoal();
        rh.action_client_ptr->waitForResult(ros::Duration(1.0));
      }
    }
...
template <typename ActionType>
template <typename Svc_T>
typename BaseFleetManager<ActionType>::result_t
BaseFleetManager<ActionType>::callService(ros::ServiceClient &sc,
                                          typename Svc_T::Request req) const {
  typename Svc_T::Response res;
  if (sc.call(req, res)) {
    if (res.success) {
      ROS_INFO_STREAM("Called service \""
                      << sc.getService() << "\" with response \"" << res.message
                      << "\".");
      return {true, res.message};
    } else {
      ROS_WARN_STREAM("Called service \""
                      << sc.getService() << "\" with response \"" << res.message
                      << "\".");
    ...
    const std::string msg =
        "Failed to call service \"" + sc.getService() + "\".";
    ROS_WARN_STREAM(msg);
    return {false, msg};
  }
}
...
template <typename Svc_T>
typename BaseFleetManager<ActionType>::result_t
BaseFleetManager<ActionType>::callService(ros::ServiceClient &sc) const {
