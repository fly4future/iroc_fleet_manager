...
template <typename Msg_T> Msg_T toRosMsg(const custom_types::Point2D &point) {
  Msg_T msg_point;
  msg_point.x = point.x;
  msg_point.y = point.y;
  return msg_point;
...
template <typename Msg_T> Msg_T toRosMsg(const custom_types::Point3D &point) {
  Msg_T msg_point;
  msg_point.x = point.x;
  msg_point.y = point.y;
  msg_point.z = point.z;
...
template <typename Msg_T> Msg_T toRosMsg(const custom_types::Reference &point) {
  Msg_T msg_point;
...
template <typename Msg_T> Msg_T toRosMsg(const custom_types::Subtask &subtask) {
  Msg_T ros_subtask;
...
template <typename Msg_T> Msg_T toRosMsg(const custom_types::Waypoint &waypoint) {
  Msg_T ros_waypoint;

  ros_waypoint.reference_point =
      toRosMsg<mrs_msgs::Reference>(waypoint.reference);

  ros_waypoint.subtasks.reserve(waypoint.getSubtasks().size());
  for (const auto &subtask : waypoint.getSubtasks()) {
    ros_waypoint.subtasks.emplace_back(
        toRosMsg<iroc_mission_handler::Subtask>(subtask));
  }
